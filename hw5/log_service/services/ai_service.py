...
class AIService:
    ...
    def detect_anomalies(self, logs_payloads: list[str]) -> tuple[str, int, int]:
        if not settings.ANTHROPIC_API_KEY:
            return "AI API key missing.", 0, 0
            
        prompt = (
            "Analyze the following log entries for anomalies, security threats, or unusual patterns (e.g., potential brute force, mass data export, unusual access times).\n"
            "Format your response as a list of identified risks and a severity score (0-10).\n"
            "Logs:\n"
        )
        prompt += "\n".join(logs_payloads)
        ...
        try:
            ...
            metrics_store.record_ai_call(latency_ms)
            tokens = response.usage.input_tokens + response.usage.output_tokens
            log.info("ai_call", action="detect_anomalies", model=settings.CLAUDE_MODEL, tokens=tokens, latency_ms=latency_ms)
            return response.content[0].text.strip(), latency_ms, tokens
        except Exception as e:
            log.error("ai_call_failed", action="detect_anomalies", error=str(e))
            return f"Error connecting to AI: {e}", 0, 0
